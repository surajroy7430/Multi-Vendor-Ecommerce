...
import { setupDeleteAccount, showToast } from "../script.js";
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (vendorDoc.exists()) {
      ...
      if (
        ...
        window.location.pathname === "/User/Cart.html" ||
        window.location.pathname === "/User/My-Orders.html"
      ) {
...
function createNewProduct() {
  ...
  if (addProductForm) {
    addProductForm.addEventListener("submit", async (e) => {
      ...
      let ratings = parseFloat(
        document.getElementById("product-ratings").value
      );
      let size = document
      ...
      if (formMode === "edit" && editingProductId) {
        ...
        await updateDoc(
          doc(db, "vendors", currentVendorId, "myproducts", editingProductId),
          productData
        );
        showToast("Product updated!");
      } else {
        let myProductRef = await addDoc(
          collection(db, "vendors", currentVendorId, "myproducts"),
          productData
        );
